...
export function Hero() {
  ...
  const [conversation, setConversation] = useState<{type: 'user' | 'ai', text: string}[]>([
    {type: 'user', text: 'How can AI help my business automate customer support?'},
    {type: 'ai', text: 'I can build custom AI agents that handle routine inquiries 24/7, learn from interactions, and seamlessly escalate complex issues to your team when needed.'}
  ]);
  ...
  useEffect(() => {
    ...
    if (conversation.length > 0 && conversation[conversation.length - 1].type === 'ai') {
  ...
  const simulateNewConversation = () => {
    ...
    setConversation([
      {type: 'user', text: userMessages[randomIndex]}
    ]);
    
    // Simulate AI thinking and then responding
    setTimeout(() => {
      setConversation(prev => [...prev, {type: 'ai', text: aiResponses[randomIndex]}]);
    }, 1500);
  };
        ...
        <div className="w-full md:w-1/2 flex flex-col justify-center">
          ...
          >
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold leading-tight md:leading-tight lg:leading-tight">
              Transforming <HeroHighlight>{typewriterText || 'Businesses'}</HeroHighlight> with
        ...
        >
          <div className="relative rounded-xl md:rounded-2xl overflow-hidden bg-black/5 p-2 md:p-4">
            <div className="absolute top-0 left-0 right-0 h-8 md:h-10 bg-background/80 backdrop-blur-sm border-b border-border/20 flex items-center px-2 md:px-4">
              ...
              <div className="mx-auto text-xs md:text-sm font-medium text-muted-foreground">AI Assistant Demo</div>
            </div>
            ...
            <div className="mt-8 md:mt-10 h-[200px] md:h-[300px] overflow-y-auto flex flex-col space-y-3 p-1 md:p-2">
              {conversation.map((msg, i) => (
                <div key={i} className={`flex ${msg.type === 'user' ? 'justify-end' : 'justify-start'}`}>
                  ...
                  <div className={`max-w-[80%] p-2 md:p-3 rounded-lg text-xs md:text-sm ${
                    msg.type === 'user' 
                      ? 'bg-primary/20 text-foreground ml-auto' 
                      : 'bg-accent text-foreground'
                  }`}>
                    {i === conversation.length - 1 && msg.type === 'ai' ? currentMessage : msg.text}
                    {i === conversation.length - 1 && msg.type === 'ai' && isTyping && (
